...
public class userAdd extends AppCompatActivity {
    ...
    private void cancelActivity() {
        ...
        new AlertDialog.Builder(userAdd.this)
                ...
                .setNegativeButton("确定", (dialog, which) -> {
                    //关闭注册页面，并返回登录页面
                    Intent intent = new Intent(userAdd.this, userList.class);
    ...
    private void registerActivity() {
        ...
        if (TextUtils.isEmpty(username)) {
            Toast.makeText(userAdd.this, "用户名不能为空", Toast.LENGTH_SHORT).show();
        ...
        if (TextUtils.isEmpty(confirm)) {
            Toast.makeText(userAdd.this, "确认密码不能为空", Toast.LENGTH_SHORT).show();
            confirE.requestFocus();
            return;
        }
        //检查密码和确认密码是否匹配
        if (!password.equals(confirm)) {
            Toast.makeText(userAdd.this, "密码不匹配", Toast.LENGTH_SHORT).show();
            confirE.requestFocus();
            return;
        }
        //获取选定的性别
        ...
        if (sex.isEmpty()) {
            Toast.makeText(userAdd.this, "性别不能为空", Toast.LENGTH_SHORT).show();
            sexR.requestFocus();
            return;
        }
        ...
        //显示成功消息
        Toast.makeText(userAdd.this, "注册成功", Toast.LENGTH_SHORT).show();
        Intent intent = new Intent(userAdd.this, userList.class);
        startActivity(intent);
        //返回登录页面
        finish();
    ...
    private boolean save(User user) {
        if (!fq.containsByName(user.getUsername())) {
            ...
            //显示成功消息
            Toast.makeText(userAdd.this, "用户名已存在", Toast.LENGTH_SHORT).show();
            return false;
        }
    }
}